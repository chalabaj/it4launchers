...
mode="movie"         # ="wigner"  - take initial conditions from Wigner distributions.  (irest=1)                     # ="movie"   - take initial geometries sequentially from XYZ movie.
...
INPUTDIR=inputsdir   # Directory with input files for ABIN
...
molname=testinglaunch                         # Name of the job in the queue
abinexe=/home/chalabaj/prog/ABIN/abin        # path to abin binary
...
jobs=2              # number of batch jobs to submit. Trajectories will be distributed accordingly.
...
if [[ $? -ne "0" ]];then
   Error "abin-randomint MyIrandom"
fi
...
k=1  #loop variable, starts with the same index as j var.
if [[ ! -z "$submit" ]];then
   cd $folder
   while [[ $k -le $j ]]
   do
