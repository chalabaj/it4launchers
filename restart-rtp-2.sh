...
PATCHED=unpatched
runtype=RTP_RESTART_2   # dont set to -SCT, WF or RTP- cause rm procedure remove already  finished runs
INPUTDIR=inputsdir-pbe
onlysub=sub   # all or just sub"
IREST=2  #i-th restart / second, first....
cd $folder
TRAJSDIR=$PWD
...
#!!!!!!!!!!!!CHANGE FOR NEXT RESTART HERE 
cp  RTP_RESTART_1/${projectname}_$ITRAJ-pos-1.xyz                RTP_RESTART_2/.
cp  RTP_RESTART_1/${projectname}_$ITRAJ-1.ener                   RTP_RESTART_2/.
cp  RTP_RESTART_1/${projectname}_$ITRAJ-RESTART.rtpwfn           RTP_RESTART_2/.
cp  RTP_RESTART_1/${projectname}_$ITRAJ-1.restart                RTP_RESTART_2/$runtype.inp
# RT RESTART folder is almost ready


cd ..  #!!!!!! now we are in folder with TRAJS

#PREPARE file which gonna be proccesed after submition
cat > TRAJ.$ITRAJ/$runtype/r.$runtype.$ITRAJ << EOF
#!/bin/bash
#PBS -V

# where to copy final data
IWORKDIR=$PWD/TRAJ.$ITRAJ/$runtype    
...
LOGFILE=${folder}_${runtype}_${ITRAJ}_\${PBS_JOBID}.log #input file
PATCHED=$PATCHED                                                         #number of CPUs
OUTFILE=$runtype.out
RUNTYPE=${runtype}

EOF
cat ../$INPUTDIR/CP2Ks-dynamics_rtp_restart >> TRAJ.$ITRAJ/$runtype/r.$runtype.$ITRAJ
chmod +x TRAJ.$ITRAJ/$runtype/r.$runtype.$ITRAJ
...
for ((i=1;i<=$ntrajs;i++))
do
 # sed -i "s/COORD_FILE_NAME ${projectname}_${i}.xyz/#COORD_FILE_NAME ${projectname}_${i}.xyz/g" TRAJ.$i/$runtype/$runtype.inp
 # sed -i "s/COORD_FILE_FORMAT  XYZ/#COORD_FILE_FORMAT  XYZ/g" TRAJ.$i/$runtype/$runtype.inp
  sed -i "s/NUMBER_OF_ATOMS  6/#NUMBER_OF_ATOMS  6/g" TRAJ.$i/$runtype/$runtype.inp
  sed -i "s/MULTIPLE_UNIT_CELL  1 1 1/#MULTIPLE_UNIT_CELL  1 1 1/g" TRAJ.$i/$runtype/$runtype.inp
  sed -i "s/MAX_ITER  5/MAX_ITER  25/g" TRAJ.$i/$runtype/$runtype.inp
done
